...
;; TODO: refactor mu4e refile function
(defun my-mu4e-refile-folder-function (msg)
  "refile while keeping folders MSG."
  (let ((maildir (mu4e-message-field msg :maildir))
        folder)
    (cond
     ((string-prefix-p "/r/" maildir)
      (setq folder (concat "/r_archive/" (substring maildir (length "/r/")))))
     ((string-prefix-p "/q/" maildir)
      (setq folder (concat "/q_archive/" (substring maildir (length "/q/")))))
     ((string-prefix-p "/e/" maildir)
      (setq folder (concat "/e_archive/" (substring maildir (length "/e/"))))))
    folder))
